const {
  ...
  const NFT = artifacts.require('Nifty');
  const Auction = artifacts.require('AuctionMock');
  const AuctionReal = artifacts.require('Auction');
  ...
  contract('Auction', (accounts) => {
    ...
    beforeEach(async () => {
      this.token = await NFT.new({ from: admin });
      ...
      describe('validation', async () => {
        ...
        it('fails if token already has auction in play', async () => {
          ...
          await expectRevert(
            ...
            'Auction.createAuction: Cannot relist'
          );
        });
    ...
    describe('withdrawBid()', async () => {
      ...
      it('fails with withdrawing a bid which does not exist', async () => {
        await expectRevert(
          ...
          'Auction.withdrawBid: You are not the highest bidder'
        );
      });
      ...
      it('fails with withdrawing when lockout time not passed', async () => {
        ...
        await expectRevert(
          ...
          "Auction.withdrawBid: Cannot withdraw until lock time has passed"
        );
      });
      ...
      describe('validation', () => {
        ...
        it('cannot result if not an owner', async () => {
          await expectRevert(
            ...
            'Auction.resultAuction: Sender must be item owner'
          );
        });
        ...
        it('cannot result if auction has not ended', async () => {
          await expectRevert(
            ...
            'Auction.resultAuction: The auction has not ended'
          );
        });
      ...
      describe('validation', async () => {
        ...
        it('cannot cancel if not an admin', async () => {
          await expectRevert(
            ...
            'Auction.cancelAuction: Sender must be item owner'
          );
        });
